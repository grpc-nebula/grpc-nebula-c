...
}; //end of PickFirst class declaration
...
void PickFirst::StartPickingLocked() {
  ...
  // size_t number = subchannel_list_->num_subchannels();
  if (subchannel_list_ != nullptr && subchannel_list_->num_subchannels() > 0) {
   ...
   //     grpc_get_subchannel_address_uri_char(subchannel_list_->subchannel(0));
    //if (strcmp(p->base.provider_addr, "") == 0 && host_info != NULL) {
    if( host_info != NULL){
      pt = const_cast<char*> (strchr(host_info, ':'));
      size = strlen(host_info) - (size_t)(pt - host_info);
      if (pt != NULL && size > 10) {
        //memcpy(p->base.provider_addr, pt + 1, size);
      }
    }
...
void PickFirst::UpdateLocked(const grpc_channel_args& args,
  ...
  // size_t num = subchannel_list->num_subchannels();
  ...
  if (selected_ == nullptr) {
    ...
    // 拿到了subchannel_list_（引用）之后，没有开始pick？？
    if (started_picking_) {
      subchannel_list_->subchannel(0)
          ->CheckConnectivityStateAndStartWatchingLocked();
    }
...
////----begin----by yang
//// select readylist
//static bool orientsec_grpc_item(ready_list* list, round_robin_lb_policy* p) {
//  const char* host_info =
//      grpc_get_subchannel_address_uri_char(list->subchannel);
//  unsigned int i = 0;
//  if (host_info == NULL) {
//    return false;
//  }
//  if (strcmp(p->base.provider_addr, "") == 0) {
//    for (i = 0; i < sizeof(p->base.provider_addr) && i < strlen(host_info);
//         i++) {
//      p->base.provider_addr[i] = host_info[i];
//    }
//    return true;
//  }
//  host_info = strchr(host_info, ':') + 1;
//  if (strcmp(p->base.provider_addr, host_info) == 0) {
//    host_info = NULL;
//    return true;
//  }
//  return false;
