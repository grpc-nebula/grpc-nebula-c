...
void RoundRobin::StartPickingLocked() {
  ...
  //JUST in reserve added by Jianbin
  //----begin----
  //// put provider_addr into policy
  //char* pt = NULL;
  //size_t size = 0;
  //
  ////获得load balance policy
  //LoadBalancingPolicy* p = subchannel_list_->policy();
  //const char* host_info =  "ipv4:192.168.2.123:55555";
  ////     grpc_get_subchannel_address_uri_char(subchannel_list_->subchannel(0));

  // /* const char* host_info =
  //    grpc_get_subchannel_address_uri_char(subchannel(index)->subchannel());*/
  // if (strcmp(p->provider_addr, "") == 0 && host_info != NULL) {
  ////if (host_info != NULL) {
  //  pt = const_cast<char*>(strchr(host_info, ':'));
  //  size = strlen(host_info) - (size_t)(pt - host_info);
  //  if (pt != NULL && size > 10) {
  //    //memcpy(p->provider_addr, pt + 1, size);
  //  }
  //}
...
bool RoundRobin::DoPickLocked(PickState* pick) {
  //获取下次调用subchannel的index，相当于1.2.4的
  // selected = peek_next_connected_locked_choose_subchannel
...
RoundRobin::RoundRobinSubchannelList::GetNextReadySubchannelIndexLocked() {
  ...
  //if (strcmp(p->provider_addr, "2.2.2.2:2222") == 0) {
  //  return num_subchannels();
  //  }
  //----end----
  for (size_t i = 0; i < num_subchannels(); ++i) {
    ...
    //debug here
    grpc_connectivity_state state = subchannel(index)->connectivity_state();
    if (state == GRPC_CHANNEL_READY) {
      ...
      // 获取lb 里面的provider ip，根据ready的subchannel中ip进行比对，
      // 如果 ready list中存在此ip，返回此index 给roundrobin pick
  ...
  /*if (subchannel(last_ready_index_)->connectivity_state() == GRPC_CHANNEL_READY){
      return last_ready_index_;
  }*/
  if (grpc_lb_round_robin_trace.enabled()) {
